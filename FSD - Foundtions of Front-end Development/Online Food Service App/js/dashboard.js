...
function displayFood(meals){
 meals.forEach(meal=> {
    ...
    foodSelection.innerHTML +=`
        <div class="bg-pink-200 p-4 rounded-lg shadow-md flex flex-col items-center">
            ...
            <h3 class="text-lg font-bold text-center">${meal.strMeal}</h3>
            <p class="text-gray-800 text-sm text-center">Price: $${price}</p>
            <input type="button" value = "Add To Cart"
                onclick="addToCart(${meal.idMeal},'${meal.strMeal}',${price})"
                class="bg-green-900 font-bold text-white py-2 rounded-md hover:bg-pink-800 transition-colors duration-300 cursor-pointer w-full mt-2" />
        </div>
    `
...
function updateCart() {
    ...
    cart.forEach(item => {
        cartItem.innerHTML += `
            <div class="bg-pink-200 rounded-lg shadow p-4 mb-4 flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                    <h3 class="text-1g font-bold mb-1">${item.mealName}</h3>
                ...
                <div class="flex space-x-2 mt-2 md:mt-0">
                    <input type="button" value="+" onClick="changeQuantity(${item.mealId}, 1)"
                        class="bg-pink-400 hover:bg-green-900 text-white font-bold py-1 px-3 rounded cursor-pointer"/>
                    <input type="button" value="-" onClick="changeQuantity(${item.mealId}, -1)"
                        class="bg-pink-700 hover:bg-green-900 text-white font-bold py-1 px-3 rounded cursor-pointer"/>
                </div>
            </div>
        `;
    ...
    if(cart.length > 0) {
        cartItem.innerHTML += `<div class="text-right mt-4"><h3 class="text-xl font-bold text-pink-300">Total Amount: <span class="text-white">$${total}</span></h3></div>`;
    }
}
...
function checkOut(){
    if(cart.length === 0){
        ...
        alert("Checkout was successful! Thank you for your order.");
